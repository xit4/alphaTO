...
# Script to perform DBSCAN clustering on multiple datasets with gridsearch on parameters
...
# read the rows from the CSV file. Skip some rows in skipfooter to reduce computational times/memory requirements
# if necessary
start = time.time()
...
for data, label in data_range:
    # DBSCAN
    # define the ranges of eps and min_samples
    ...
    # cycle on the ranges and compute ARI and Silhouette
    for min_samples in min_samples_range:
        ...
        # plot ARI and Silhouette for every min_samples value
        plt.plot(eps_range, ARI, label="ARI-minsample{}".format(min_samples))
        plt.plot(eps_range, Silhouette, label="Silhouette-minsample{}".format(min_samples))

    # save the plot with every line that was printed
    plt.legend()
