...
for data, label in data_range:
    ...
    # cycle through all the parameter ranges
    for max_iter in max_iter_range:
        for n_init in n_init_range:
            ...
            # plot ARI and Silhouette on the figure
            plt.plot(n_clusters_range, ARI, label="ARI-max{}init{}".format(max_iter, n_init))
            plt.plot(n_clusters_range, Silhouette, label="Silhouette-max{}init{}".format(max_iter, n_init))

    # save the figure with all the plots of ARI and Silhouette computed for every parameter in the parameter range
    plt.legend()
